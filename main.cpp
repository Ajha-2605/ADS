...
int main(int argc, char* argv[])
{
	...
	try
	{
		...
		{
			value = line.substr(11, line.size() - 11 - 1);
			m = stoi(value);
	...
	while (getline(file, line))
	{
		...
		if(0 == line.compare(0, 6, "Insert"))
		{
			...
			value = line.substr(pos + 2, line.size() - pos - 4);
		...
		else if (0 == line.compare(0, 6, "Search"))
		{
			...
			else
			{
				...
				key2 = atof(line.substr(pos + 2, line.size() - pos -3).c_str());
		...
		{
			key = atof(line.substr(8, line.size() - 8 - 1).c_str());
			tree.Delete(key);
		}
	}
